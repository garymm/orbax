...
class _ControllerSaveHandoffLifecycle:
  ...
  def _drain_handoff(self, pending_save: _PendingSave) -> None:
    """Drains a worker handoff result in the background."""
    try:
  ...
  def drain(self, *, wait: bool = True) -> bool:
  ...
  def _drain(self, pending_save: _PendingSave, *, wait: bool) -> bool:
    ...
    with pending_save.drain_lock:
      ...
      if saved is None:
        raise RuntimeError(
            'Pathways colocated MTC save handoff completed without a worker '
            'save result.'
        )
      if saved:
  ...
  def raise_on_handoff_error(self) -> None:
...
class ColocatedController:
  ...
  def _drain_pending_save(self, *, wait: bool = True) -> bool:
    return self._save_lifecycle.drain(wait=wait)

  def _check_pending_save_handoff_error(self) -> None:
    self._save_lifecycle.raise_on_handoff_error()
