...
r"""One-shot prep for the safetensors load benchmark: stage a model + spec.
...
(`--sharding-out`, one `--strategy` per run). Example:
  ...
  #   python -m orbax.checkpoint._src.testing.benchmarks.safetensors.prepare \\
...
authenticated `gcloud` storage for `--gcs`.
"""
...
# TODO(b/519204863): Reuse orbax safetensors code for stuff like this.
def _read_safetensors_header(path: epath.Path) -> dict[str, Any]:
...
def build_config(
    model_dir: epath.Path, axis_size: int, strategy: str
) -> dict[str, Any]:
...
def _snapshot_download(repo_id: str, dest: epath.Path) -> epath.Path:
  ...
  try:
    import huggingface_hub as hf_hub  # pylint: disable=g-import-not-at-top
...
def _mirror_to_gcs(local_dir: epath.Path, gcs_uri: str) -> None:
  """Mirrors a model dir to GCS via `gcloud storage rsync` (idempotent)."""
  subprocess.run(
      [
          "gcloud",
          "storage",
...
_SHARDING_OUT = flags.DEFINE_string(
    "sharding-out", None, "Write the sharding JSON here (local or gs://)."
)
_AXIS_SIZE = flags.DEFINE_integer(
    "axis-size",
    None,
    "Mesh axis size (= participating devices); needs --sharding-out.",
)
...
def main(argv: list[str]) -> None:
  ...
  if _REPO.value:
    if _DOWNLOAD_DIR.value:
      ...
      dest = (
          epath.Path(tempfile.gettempdir())
          / "safetensors-prepare-model"
          / model_slug
      )
